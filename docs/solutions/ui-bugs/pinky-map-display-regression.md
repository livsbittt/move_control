...
## 최신 검증: 정지 상태 지도 표시 GO

2026-09-07 22:55 KST에 `pinky-calibration-1c40085` 실행본도 수정하고
기존 ROS launch 관리자가 재시작한 PID 59278에서 다시 확인했다.
수정 코드는 `055ed2b`의 렌더러와 현재 main의 연속 좌표 보정을 반영했다.
단독 웹 프로세스를 추가로 실행하지 않았다.

- ROS 지도와 웹 PNG가 픽셀 단위로 일치했다.
- 로봇 중심은 (12, 26) 셀의 빈 공간에 있고, 실제 웹에서도 통로 안에 표시됐다.
- 웹 위치와 TF 위치 차이는 약 0.39mm였다. 이는 두 소프트웨어 좌표의 차이이며
  실제 바닥에서의 절대 위치 정확도를 측정한 값은 아니다.
- 라이다 664개 점과 지도 벽의 거리 중앙값은 1.11cm, 90백분위는 1.70cm였고,
  모든 점이 벽에서 4cm 이내였다. 정지 상태의 한 표본이다.
- 브라우저 오류 없음, 비상정지 유지, 속도 0. 표시 각도는 약 -22.9도였다.

[최신 화면](../../evidence/2026-09-07-map-display/current-verified.png),
[최신 수치·해시](../../evidence/2026-09-07-map-display/current-verified.json),
[최신 실행본 변경 기록](../../evidence/2026-09-07-map-display/current-patch.json).
추후 다른 배포가 진행되면 이 결과를 재사용하지 말고 해당 실행본을 다시 검사한다.

## 이전 재덮임: 실행본 유지 HOLD였던 시점
...
## 2026-09-07 23:31 실제 웹 구조 수정 및 통합

최종 실행 코드 커밋은 `5ef0569`다. 최신 보정 `7781a59`, 배포 파이프라인,
동시에 실제 기기에 올라온 `fd26eec`의 모드 선택 개선까지 통합했다.
웹 실행 경로는 `/home/pinky/pinky-calibration-fd26eec`지만, 해당 소스를
`5ef0569` 아카이브로 갱신하고 colcon 빌드했다. 변경된 실행 파일은 웹 노드와
HTML뿐이므로 웹 프로세스만 재시작해 기존 SLAM 지도를 유지했다.
원본 전체 소스 백업은 `/home/pinky/map-release-records/5ef0569/before.tar`다.

구조적으로 확인한 재발 경로와 수정:

- PNG와 지도 크기·원점이 별도 잠금에서 갱신됐다. 이제 하나의 잠금에서
  이미지·세대·메타데이터를 함께 게시한다.
- `/map.png?g=N`이 세대를 무시했다. 이제 불일치 요청은 409로 거부한다.
- 새 지도 크기·원점으로 이전 이미지를 늘려 그렸다. 이전 이미지는 좌표
  메타데이터가 동일한 경우에만 재사용한다.
- 새로고침 때 현재 로봇 yaw로 화면을 다시 돌렸다. 기본 방향은 0도로
  고정하고 명시적인 방향 정렬 버튼만 회전시킨다. 실제 TF에 임의 30도 보정은 넣지 않는다.
- 프로세스 재시작으로 세대 번호가 재사용됐다. 실행 UUID를 상태·이미지 요청에
  포함하고 실행 변경 시 브라우저 캐시를 무효화한다.
- TF 조회 실패를 숨겨 진단하기 어려웠다. `pose_tf_error`로 구체적인 원인을 제공한다.

검증: 기기에서 전체 선택 테스트 208개, 최종 모드 제한을 포함한 웹·TF 테스트
21개, UI 테스트 14개 통과. 실제 URL을 Playwright로 열어 페이지 오류 0개 확인.
사용자 요청으로 Gazebo 검증은 중단했으며 아래 최종 판정은 실제 Pinky 표본이다.

실제 웹 두 번 연속 표시 검사 통과: PNG는 ROS 북쪽 행과 일치하고 로봇 셀은
빈 공간이다. 웹/TF 차이 약 0.465mm, 라이다 벽 거리 중앙값 8.26~8.39mm,
90백분위 12.07~12.36mm. 마지막 상태는 비상정지, 속도 0이다.
정지 상태의 센서·지도 정합 검증이며 전체 주행/들어 옮긴 뒤 재위치 검증은 아니다.

증거: [실제 화면](../../evidence/2026-09-07-map-structural/candidate.png),
[첫 검사](../../evidence/2026-09-07-map-structural/report.json),
[반복 검사](../../evidence/2026-09-07-map-structural/repeat.json),
[배포 기록](../../evidence/2026-09-07-map-structural/deployment.json).

추가 교훈: 배포 경로 이름을 버전 증거로 삼지 않는다. 실행 파일 해시와 통합
커밋을 대조하며, ROS 검사에는 실행 프로세스의 실제 환경을 사용한다.
이번 실행의 ROS domain은 기본값 0이며 `.bashrc`의 13을 추정 적용하면 지도 수신이 실패한다.
