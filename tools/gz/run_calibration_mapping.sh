...
for name in ('map.pgm','map.yaml','map_grid.npz','calibration.json','live_evidence.json'):
...
root=ET.parse('tools/gz/pinky_maze.sdf')
model=root.find(".//model[@name='pinky']")
model.find('pose').text='2.55 0.60 0.01 0 0 0'
